...
uint32_t RoscoinChecksum(uint8_t* p, uint32_t nBytes)
{
    if (!p || nBytes == 0)
        return 0;
...
{
    uint32_t checksum = RoscoinChecksum(&data[0], data.size());
...
bool VerifyChecksum(const data_chunk& data)
{
    ...
    return RoscoinChecksum((uint8_t*)&data[0], data.size()-4) == checksum;
};
