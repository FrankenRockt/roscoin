...
QVariant AddressTableModel::data(const QModelIndex &index, int role) const
{
    ...
    else if (role == Qt::FontRole)
    {
        ...
        {
            font = GUIUtil::roscoinAddressFont();
        }
        return font;
    }
...
bool AddressTableModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case Label:
            ...
            {
                wallet->SetAddressBookName(CRoscoinAddress(strTemp).Get(), value.toString().toStdString());
            }
        ...
        case Address:
            ...
            if(CRoscoinAddress(rec->address.toStdString()) == CRoscoinAddress(value.toString().toStdString()))
            {
                editStatus = NO_CHANGES;
                return false;
            }
            ...
            else if(wallet->mapAddressBook.count(CRoscoinAddress(value.toString().toStdString()).Get()))
            {
            ...
            {
                {
                    LOCK(wallet->cs_wallet);
                    // Remove old entry
                    wallet->DelAddressBookName(CRoscoinAddress(rec->address.toStdString()).Get());
                    // Add new entry with new address
                    wallet->SetAddressBookName(CRoscoinAddress(value.toString().toStdString()).Get(), rec->label.toStdString());
                }
            }
            break;
        }
...
{
    // Update address book model from Roscoin core
...
QString AddressTableModel::addRow(const QString &type, const QString &label, const QString &address, int addressType)
{
    ...
    if (type == Send)
    {
        ...
        } else
        {
            ...
            {
                LOCK(wallet->cs_wallet);
                if (wallet->mapAddressBook.count(CRoscoinAddress(strAddress).Get()))
                {
                    editStatus = DUPLICATE_ADDRESS;
                    return QString();
                };
                
                wallet->SetAddressBookName(CRoscoinAddress(strAddress).Get(), strLabel);
        }
    }
    }else if(type == Receive)
    {
        ...
        } else
        {
            ...
            strAddress = CRoscoinAddress(newKey.GetID()).ToString();
...
QString AddressTableModel::labelForAddress(const QString &address) const
{
    {      
        ...
        {
            CRoscoinAddress address_parsed(sAddr);
