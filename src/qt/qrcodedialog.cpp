...
{
    QString ret = QString("Roscoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ROS unit input in lnReqAmount, we generate the URI with ROS as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(RoscoinUnits::format(RoscoinUnits::ROS, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
