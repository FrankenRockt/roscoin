...
bool WalletModel::validateAddress(const QString &address)
{
    ...
    CRoscoinAddress addressParsed(sAddr);
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
	...
	
    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            CScript scriptPubKey;
            scriptPubKey.SetDestination(CRoscoinAddress(sAddr).Get());
            vecSend.push_back(make_pair(scriptPubKey, rcp.amount));
...
static void NotifyAddressBookChanged(WalletModel *walletmodel, CWallet *wallet, const CTxDestination &address, const std::string &label, bool isMine, ChangeType status)
{
    ...
    {
        OutputDebugStringF("NotifyAddressBookChanged %s %s isMine=%i status=%i\n", CRoscoinAddress(address).ToString().c_str(), label.c_str(), isMine, status);
        QMetaObject::invokeMethod(walletmodel, "updateAddressBook", Qt::QueuedConnection,
                                  Q_ARG(QString, QString::fromStdString(CRoscoinAddress(address).ToString())),
                                  Q_ARG(QString, QString::fromStdString(label)),
                                  Q_ARG(bool, isMine),
                                  Q_ARG(int, status));
    }
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        mapCoins[CRoscoinAddress(address).ToString().c_str()].push_back(out);
    }
}
